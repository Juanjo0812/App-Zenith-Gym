...
const SplashScreen = ({ navigation }: Props) => {
  ...
  useEffect(() => {
    ...
    Animated.timing(percentAnim, {
      ...
      if (finished) {
        setTimeout(async () => {
          const demoMode = await isDemoModeEnabled().catch(() => false);
          if (demoMode) {
  ...
  return (
    <View style={styles.container}>
            ...
            const dashOffsetAnim = traceAnims[i].interpolate({
              ...
              return (
                <G key={`trace-bg-${i}`}>
                  ...
                  <Path d={trace.path} stroke="#00E5FF" strokeWidth="1" fill="none" strokeLinecap="square" strokeLinejoin="miter" opacity={0.3} />
                  ...
                  <Circle cx={trace.endX} cy={trace.endY} r={3} fill={activeNodes[i] ? "#33EEFF" : "#00E5FF"} opacity={activeNodes[i] ? 1 : 0.25} />
                  {activeNodes[i] && (
      ...
      <Animated.View style={[styles.sceneArea, { opacity: fadeIn, zIndex: 10 }]}>
        {/* ── Ambient cyan glow behind the V ── */}
        ...
        <Animated.View style={[styles.svgLayer, { transform: [{ rotate: spin }] }]}>
          <Svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`}>
            ...
            <Circle
              ...
              stroke="#00E5FF" strokeWidth="16" fill="none" opacity={0.08}
            />
            {/* Mid glow ring */}
            <Circle
              cx={CENTER} cy={CENTER} r={OUTER_R}
              stroke="#33EEFF" strokeWidth="6" fill="none" opacity={0.2}
              strokeLinecap="round"
            ...
            <Circle
              cx={CENTER} cy={CENTER} r={OUTER_R}
              stroke="#00E5FF" strokeWidth="3" fill="none" opacity={0.9}
              strokeLinecap="round"
        ...
        <Animated.View style={[styles.svgLayer, { opacity: sparkFlicker1, transform: [{ rotate: spin }] }]}>
          <Svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`}>
            <Path d={arcSegments[0]} stroke="#33EEFF" strokeWidth="2.5" fill="none" strokeLinecap="round" strokeLinejoin="round" />
        ...
        <Animated.View style={[styles.svgLayer, { opacity: sparkFlicker2, transform: [{ rotate: reverseSpin }] }]}>
          <Svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`}>
            <Path d={arcSegments[2]} stroke="#00E5FF" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
            <Path d={arcSegments[3]} stroke="#33EEFF" strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round" opacity={0.8} />
          </Svg>
        </Animated.View>

        <Animated.View style={[styles.svgLayer, { opacity: sparkFlicker3 }]}>
          <Svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`}>
            <Path d={arcSegments[4]} stroke="#00E5FF" strokeWidth="2.5" fill="none" strokeLinecap="round" strokeLinejoin="round" />
          </Svg>
        </Animated.View>
      ...
      <Animated.View style={[styles.titleContainer, { opacity: fadeIn }]}>
        <Text style={styles.brandTitle}>ZENITH</Text>
        <Text style={styles.brandSubtitle}>Entrena con claridad</Text>
      </Animated.View>
    </View>
  );
};
